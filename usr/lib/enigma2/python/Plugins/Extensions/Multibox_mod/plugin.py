...
config.plugins.Multibox_mod = ConfigSubsection()
config.plugins.Multibox_mod.refreshInterval = ConfigNumber(default=10)
config.plugins.Multibox_mod.woeid = ConfigNumber(default = 638242)
config.plugins.Multibox_mod.tempUnit = ConfigSelection(default="Celsius", choices = [
				("Celsius", _("Celsius")),
				("Fahrenheit", _("Fahrenheit"))
				])
				
def Plugins(**kwargs):
	return [PluginDescriptor(name=_("Multibox_mod FHD Configtool"), description=_("Personalize your Multibox_mod FHD (Skin by speedy005)"), where = [PluginDescriptor.WHERE_PLUGINMENU],
	icon="plugin.png", fnc=main)]

def main(session, **kwargs):
	if config.skin.primary_skin.value == "Multibox_mod/skin.xml":
		session.open(Multibox_mod_Config)
	else:
		AddPopup(_('Please activate Multibox_mod FHD Skin before run the Config Plugin'), type=MessageBox.TYPE_ERROR, timeout=10)
		return []
...
class Multibox_mod_Config(Screen, ConfigListScreen):

	skin = """
		<screen name="Multibox_mod_Config" position="center,center" size="1280,720" title="Multibox_mod Setup" >
			<widget source="Title" render="Label" position="70,47" size="950,43" font="Regular;35" transparent="1" />
	...
	def keyYellow(self):
		if self.myAtileHD_active.value:
			self.session.openWithCallback(self.Multibox_modScreenCB, Multibox_modScreens)
		else:
	...
	def select_weather_id_callback(self, res):
		if res and isInteger(res):
			print res
			config.plugins.Multibox_mod.woeid.value = int(res)
	...
	def about(self):
		self.session.open(Multibox_mod_About)
...
class Multibox_mod_About(Screen):
...
class Multibox_modScreens(Screen):

	skin = """
		<screen name="Multibox_modScreens" position="center,center" size="1280,720" title="Multibox_mod Setup">
			<widget source="Title" render="Label" position="70,47" size="950,43" font="Regular;35" transparent="1" />
